...
\begin{definition}
Любой кубит можно представить в виде
\[
...
\begin{problem}
  ...
  \begin{enumerate}
    ...
    \item Изобрази на сфере Блоха $NOT(\ket{0})$, $NOT(\ket{1})$, $NOT(\ket{+})$, $NOT(\ket{-})$;
    \item Какое преобразование делает $NOT$ на сфере Блоха?
  \end{enumerate}
  \begin{sol}
  \end{sol}
\end{problem}
...
\begin{problem}
Как для произвольных кубитов связаны результаты умножения $\braket{a}{b}$ и $\braket{b}{a}$?
  \begin{sol}
  \end{sol}
\end{problem}
...
Известно, что комплексное число $r_{\gamma}$ при умножении на другие комплексные числа
  ...
  \begin{enumerate}
    ...
    \item После измерения кубит $\ket{q}$ оказывается в состоянии $\ket{0}$ с вероятностью $0.75$.
    Каковы вероятности состояний $\ket{0}$ и $\ket{1}$ при измерении кубита $r_{\gamma}\ket{q}$?
    \item Сколькими способами можно дополнить кубит $\ket{q}$ до ортонормального базиса?
  \end{enumerate}
\begin{sol}
\end{sol}
\end{problem}
