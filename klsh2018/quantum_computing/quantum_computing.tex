...
\begin{problem}
С помощью классических логических элементов $NOT$, $AND$, $OR$ реализуй классические элементы:
\begin{enumerate}
  \item исключающее ИЛИ, $XOR$;
  \item управляемое НЕ, $CNOT$;
\end{enumerate}
\begin{sol}
...
Если измерить кубит $\ket{q} = \alpha_0 \ket{0} + \alpha_1 \ket{1}$, то он
