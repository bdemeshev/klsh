...
if (!AoPS.hasOwnProperty('Ui') || !AoPS.hasOwnProperty('ui_main_loaded')) {
			...
			fitMasks : function() {		
				...
				function makeFinalAdjustments() {
					...
					_.each(this.active_modals, function(modal) {
						...
						// PR 2016-09-26 made this Mac Chrome OS X Hack
						// PM 2016-09-26 undid it so he could do something else
						//if (navigator.userAgent.indexOf('Mac OS X') && !!window.chrome && !!window.chrome.webstore) {
						//	modal.$obj.css({
						//		'position' : 'fixed'
						//	});
						//} else {
						...
						modal.$obj.css({
							...
							//'right' : parseInt(Math.max(0, ($(window).width() - modal.$obj.outerWidth()) / 2))
						});
	...
	//				priv.slider_position = Math.floor(priv.slider_position);
				...
				setBarVisibility : function (is_visible) {
					...
					if (is_visible && priv.chrome53_hack) {
